...
/**
  ...
  * @param parser           The body parser to use
  */
class NonceActionBuilder @Inject()(settings: RecaptchaSettings, nonceGenerator: NonceGenerator,
      parser: BodyParsers.Default)(implicit ec: ExecutionContext) extends ActionBuilderImpl(parser) {
  ...
  /**
    * Adds a generated nonce to the request (as a request attribute), and adds a corresponding policy to the response.
  ...
  override def invokeBlock[A](request: Request[A], block: (Request[A]) => Future[Result]): Future[Result] = {
    ...
    block(newRequest).map(_.withHeaders(SecurityHeadersFilter.CONTENT_SECURITY_POLICY_HEADER -> policy))
  }
}
...
class NonceGenerator @Inject()(settings: RecaptchaSettings) {
  ...
  /**
    ...
    * @return Random numbers and upper/lower case alphabetic characters
    */
