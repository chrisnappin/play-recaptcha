...
class WidgetHelper @Inject() (settings: RecaptchaSettings):
  ...
  /** Resolves any recaptcha error messages (from message key to message text) and associates it
    * with the specified fieldname (instead of the default of
    * <code>com.nappin.play.recaptcha.error</code>).
    *
    * @param fieldname
    ...
    form
      ...
      .map(e => {
        e.message match {
          ...
          case RecaptchaErrorCode.responseMissing =>
            Some(messagesProvider.messages("error.required"))
  ...
  /** Formats a CSS class attribute, including the specified main class and any further classes
    * defined in args.
    *
    * @param mainClass
    *   The main class
  ...
  def formatOtherAttributes(args: (Symbol, String)*): String =
    args
      .filter((t) => t._1 != Symbol("class"))
      .map((t) => t._1.name + "=\"" + t._2 + "\"")
      .mkString(" ")

  /** Outputs the nonce HTML attribute (name and value), if a nonce has been set as a request
    * attribute.
    * @param request
    *   (implicit) The current request
    * @return
  ...
  /** Maps the current Play locale to the reCAPTCHA v2 language code.
    ...
    *   The play locale
    * @return
    *   The language code, possibly with country code too
    */
